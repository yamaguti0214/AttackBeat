...
public class CheckNotes : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!ESCButton.Pause)
        {
            ...
            {
                soundPlay.SEPlay();
                if (fKeyLockTimer <= 0) Judge(true); // true = F�L�[
            }
            // H�L�[����
            else if (Input.GetKeyDown(KeyCode.H))
            {
                soundPlay.SEPlay();
                if (hKeyLockTimer <= 0) Judge(false); // false = H�L�[
            }
    ...
    void Judge(bool isFKey)
    {
        ...
        foreach (var note in notes)
        {
            ...
            // �����[���ɂ�锻��̃t�B���^�����O
            if (note.lane == 0 && !isFKey) continue; // �m�[�c�Ȃ̂�H��������
            if (note.lane == 1 && isFKey) continue;  // �΃m�[�c�Ȃ̂�F��������
            // note.lane == 2 (�A�Ńm�[�c) �̏ꍇ�́AF�ł�H�ł��X���[���ĉ��̏�����

            // ���A�Ńm�[�c(lane=2)��p�̔��胍�W�b�N
            if (note.lane == 2)
            {
                // ���݂̎��Ԃ��A�A�Ńm�[�c�́u�n�_�v����u�I�_�v�̊ԂɎ��܂��Ă��邩�`�F�b�N
                if (currentTime >= note.timing - goodRange && currentTime <= note.timing + note.length + goodRange)
                {
                    // �����������΁A���̏�ŘA�Ő����Ƃ��ĉ��_�i�m�[�c�͏����Ȃ��j
                    Perfect++;
                    ShowResult("Perfect");
                    NotesEffect("Perfect");
                    FullAttack += 2; // �A�Œ���1�ł��Ƃ̃X�R�A
                    audioSource.PlayOneShot(PerfectSound);
                    return; // �A�Ńm�[�c�̏����͂����ő��I���i���̒P���������ɍs�����Ȃ��j
                }
                continue;
            }
    ...
    void CheckMiss()
    {
        ...
        for (int i = notes.Count - 1; i >= 0; i--)
        {
            ...
            // �A�Ńm�[�c(lane 2)�̏ꍇ�A�I�_(timing + length)����ɂ���
            float expirationTime = (note.lane == 2) ? (note.timing + note.length) : note.timing;

            // ���胉�C���i����ԁj�� goodRange �������ʂ�߂����珈��������
            if (currentTime - expirationTime > goodRange)
            {
                // �A�Ńm�[�c(lane 2)�̏ꍇ�̏���
                if (note.lane == 2)
                {
                    // �A�Ńm�[�c�́A�ʂ�߂������u�A�Œn�т𖳎��Ɋ��������v�Ƃ����Ӗ��Ȃ̂ŁA
                    // Miss�ɂ����A�X�R�A�Ȃǂ̒ʒm���o�����ɁA�Â��ɃI�u�W�F�N�g�������������܂��B
                    Debug.Log("�A�Ńm�[�c������ɏI�����܂����B");
                ...
                // --- �ʏ�m�[�c(lane 0, 1)�̏ꍇ�͍��܂Œʂ�Miss�ɂ��� ---
                Debug.Log("MISSTIMING (�ʏ�m�[�c�̒@������)");
...
IEnumerator FlyNoteCoroutine(GameObject noteObj)
{
    ...
    while (time < duration)
    {
        ...
        // 軽い放物線
        float y = (-4f * 4f * (t - 0.5f) * (t - 0.5f)) + 4f;
